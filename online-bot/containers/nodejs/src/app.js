...
// Capture location update from a user (usually triggerd by cronjob)
...
app.post("/api", function(req, res){
	...
	if (config.verification_token == data.token) {
		...
		var callback = data.callback_id.split("."),
			target = callback[0],
			action = callback[1],
			id = callback[2];
...
// Callback page after authorizations
app.get("/callback", function(req, res) {
	var temp_code = req.query.code;

	// Perform GET-request for user to obtain access_token
		...
		var request = port.request(options, function(response) {
			...
			response.on('end', function() {
				...
				if (obj.ok) {
					...
					models.User.findOneAndUpdate({token: obj.access_token}, {
